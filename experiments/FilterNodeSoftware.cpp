...
static void TransferComponents(
  ...
  //volatile char buffer[4096]; //lishay
  
 // printf("\nhight: %d width: %d bytesperpixel: %d \n",size.height,size.width,BytesPerPixel );
  pid_t pid = getpid();
  printf("\n%lu", pid);
  //printf("\n%llu", &buffer[800]); // lishay
  //printf("\n%llu", &aLookupTables[0][0]);
  printf("\n%llu", &aLookupTables[0][100]);
  printf("\n%llu", &aLookupTables[0][170]);
  //printf("\n%llu", &aLookupTables[0][255]);
  fflush(stdout); //lishay
...
//printf("\nhight: %d width: %d bytesperpixel: %d \n",size.height,size.width,BytesPerPixel );   
// printf("\nsourceStride %d, targetStride %d",sourceStride,targetStride); 
for(int t = 0; t <1; t ++){ 
    ...
    for (int32_t x = 0; x < size.width; x++) {
      ...
      for (uint32_t i = 0; i < BytesPerPixel; i++) {
        ...
        targetData[targetIndex + i] =
            aLookupTables[i][sourceData[sourceIndex + i]]; //this is where accessing table times = hight*width
 //	if(i == 1)
//	printf("\ns %02X ,t %02X",sourceData[sourceIndex + i],targetData[targetIndex + i]);
      }
...
//allocating lookup table
  ...
  //printf("geneerated lookuptable %llu",lookupTables); //lishay
